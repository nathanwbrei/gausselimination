...
int main(int argc, char** argv) {
	...
	if(rank == 0) {
		...
		for(i = 1; i < size; i++){
			...
			MPI_Isend(&columns, 1, MPI_INT, i, 0, MPI_COMM_WORLD, &request_2);
		}
	...
	if (rank == 0){
	    for(i = 1; i < size; i++){
		   	MPI_Isend((matrix_1D_mapped + (i * (local_block_size * rows))), (local_block_size * rows), MPI_DOUBLE, i, 0, MPI_COMM_WORLD, &request);
			MPI_Isend((rhs + (i * local_block_size)), local_block_size, MPI_DOUBLE, i, 0, MPI_COMM_WORLD, &request_2);
		}
	}
	...
	if(rank == 0) {	
		...
		MPI_Irecv(matrix_local_block, local_block_size * rows, MPI_DOUBLE, 0, 0, MPI_COMM_WORLD, &request);
		MPI_Irecv(rhs_local_block, local_block_size, MPI_DOUBLE, 0, 0, MPI_COMM_WORLD, &request_2); 
	}
